...
function mergefieldconfigs() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    ...
    merged.functions.push({
      ...
      aggregateFunc:
        i === 0 ? nnconfig.aggregateFunc : nnconfig.aggregateFunc ? nnconfig.aggregateFunc() : null,
      formatFunc:
        i === 0 ? nnconfig.formatFunc : nnconfig.formatFunc ? nnconfig.formatFunc() : null,
    });
  }
  ...
  return new Field(
    {
      ...
      subTotal: {
        ...
        collapsed:
          getpropertyvalue('collapsed', merged.subtotals, false) &&
          getpropertyvalue('collapsible', merged.subtotals, true),
      },
...
module.exports.config = function (config) {
  ...
  this.rowFields = (config.rows || []).map(function (fieldconfig) {
    ...
    return createfield(
      ...
      axe.Type.ROWS,
      fieldconfig,
      getfield(self.allFields, fieldconfig.name)
    );
  });
  ...
  var runtimeVisibility = {
    subtotals: {
      ...
      columns:
        self.columnSettings.subTotal.visible !== undefined
          ? self.columnSettings.subTotal.visible
          : true,
    },
  };
  ...
  this.availablefields = function () {
    return self.allFields.filter(function (field) {
      ...
      return (
        self.dataFields.every(notequalfield) &&
      ...
      if (oldaxe || newaxe) {
        ...
        if (oldaxe) {
          ...
          if (oldaxetype === newaxetype) {
            if (
              (oldposition == oldaxe.length - 1 && position == null) ||

    ...
    return React.createElement(
      'div',
      {
        className:
          ...
          (buttons.length === 0 ? ' drp-trgt-empty' : ''),
        style: style,
      },
      React.createElement(
        'table',
...
module.exports.PivotTableUpperButtons = React.createClass({
  ...
  render: function () {
    ...
    return React.createElement(
      ...
      React.createElement('tbody', null, fieldsDropTarget, dataDropTarget)
    );
  },
});
  ...
  render: function () {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('tbody', null, columnHeaders)
      )
    );
  },
});
  ...
  render: function () {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('tbody', null, rowHeaders)
      )
    );
  },
});
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(Dropdown, {
              ...
              selectedValue:
                currentFilter && currentFilter.operator
                  ? currentFilter.operator.name
                  : filtering.Operators.MATCH.name,
...
function FilterManager(reactComp, initialFilterObject) {
  ...
  this.init = function (filterContainerElement) {
    ...
    elems.checkboxes = {};
    elems.searchBox =
      elems.filterContainer.rows[0].cells[2].children[0].rows[0].cells[0].children[0];
    elems.clearSearchButton =
      elems.filterContainer.rows[0].cells[2].children[0].rows[0].cells[1].children[0];
    ...
    resizeManager = new ResizeManager(
      elems.filterContainer.parentNode,
      elems.filterContainer.rows[1].cells[0].children[0],
      elems.resizeGrip
    );
  ...
  function ResizeManager(outerContainerElem, valuesTableElem, resizeGripElem) {
    ...
    this.resizeMouseMove = function (e) {
      ...
      var offset = {
        x:
          outerContainerWidth <= minContainerWidth && e.pageX < resizeGripSize.left
            ? 0
            : e.pageX - mousedownpos.x,
        y:
          outerContainerHeight <= minContainerHeight && e.pageY < resizeGripSize.top
            ? 0
      ...
      if (newContainerHeight >= minContainerHeight) {
        ...
        valuesTableElem.style.height =
          valuesTableSize.bottom - valuesTableSize.top + offset.y + 'px';
      }
  ...
  this.toggleRegexpButtonState = function () {
    elems.enableRegexButton.className = elems.enableRegexButton.className.replace(
      'srchtyp-col-active',
      ''
    );
    if (isRegexMode) {
    ...
    if (!isSearchMode && !elems.allCheckbox.indeterminate) {
      ...
      if (checkedCount === 0) {
        ...
        for (i = 0; i < reactComp.values.length; i++) {
          ...
          if (checkboxVisible(checkbox)) {
            if (
              (!excludeUnchecked && checkbox.checked) ||
              (excludeUnchecked && !checkbox.checked)
            ) {
